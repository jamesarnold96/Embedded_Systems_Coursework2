...
volatile uint64_t newRev;
volatile uint64_t maxSpeed;
uint32_t newTorque;
...
void motorCtrlFn(){
    ...
    while(1){
        ...
        motorVelocity = abs(oldmotorPosition - motorPosition)*10;
        oldmotorPosition = motorPosition;
        counter++;
        if(counter == 10){
            counter = 0;
            printf("position: %d , velocity: %d",motorPosition, motorVelocity);
            }
        }
    }
